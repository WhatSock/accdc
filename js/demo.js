...
$A.bind("input.morphBtn", "click", function(ev) {
    ...
    $A.morph($A.reg.mcDemo, obj, {
      ...
      runAfter: function(dc) {
        ...
        dc.onDragEnd = function(ev, dd, dc) {
          ...
          if (!dc.dropped) dropIntoTheSun(dc);
        ...
        dc.accDCObj.parentNode.appendChild(
          $A.createEl(
            "div",
            null,
            {
  ...
  $A.announce(
    ($A.getEl("morphI").innerHTML =
      "Converted all images into draggable AccDC Objects")
  );
...
function dropIntoTheSun(dc) {
  var sun = firstChild($A.reg.sun.containerDiv, "img"),
    planet = firstChild(dc.containerDiv, "img"),
    additional = sun.offsetHeight / 2,
    sunOS = $A.xOffset(sun);
