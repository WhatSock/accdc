...
var firstChild = function(e, t) {
  ...
  getClosest = function(start, targ) {
    while (start) {
      ...
      if (typeof targ === "string") {
        if (start.nodeName.toLowerCase() === targ.toLowerCase()) return start;
      } else if (targ.nodeType === 1) {
        if (start == targ) return start;
      }
    }
    ...
    for (var i = 0; i < hs.length; i++) {
      ...
      if (ph && !$A.hasClass(h, "skip")) {
        ...
        var a = $A.createEl(
          ...
          null,
          h.id,
          document.createTextNode(h.innerText || h.textContent)
        );
  ...
  elastic = function(obj, maxheight) {
    if (!obj || obj.nodeName.toLowerCase() !== "textarea") return obj;
    var mimics = [
        "paddingTop",
        "paddingRight",
        "paddingBottom",
    ...
    var lineHeight =
        parseInt($A.css($textarea, "line-height"), 10) ||
        parseInt($A.css($textarea, "font-size"), 10),
      minheight = 30,
      maxheight =
        maxheight ||
        parseInt($A.css($textarea, "max-height"), 10) ||
        Number.MAX_VALUE,
      exceeded = false,
      updating = false;

    function update() {
      var txw = $textarea.offsetWidth,
        tww = $twin.offsetWidth;

      if (txw != tww) $A.css($twin, "width", $A.css($textarea, "width"));
      var text = $textarea.value
          .replace(/&/g, "&amp;")
          .replace(/ {2}/g, "&nbsp;")
          .replace(/<|>/g, "&gt;")
          .replace(/\n/g, "<br />"),
        html = $twin.innerHTML.replace(/<br>/gi, "<br />");
      ...
      if (text + "&nbsp;" !== html) {
        ...
        if (twh <= maxheight) {
          ...
          else $A.css($textarea, "height", minheight);
  ...
  transition = function(ele, targ, config) {
    ...
    for (t in targ) {
      if (t == "transform") start[t] = 0;
      else start[t] = parseInt($A.css(ele, t));
      ...
      (tmr = setInterval(function() {
        ...
        if (elapsedTime < uTotalTime) {
          ...
          for (s in start) {
            ...
            else $A.css(ele, s, nw[s]);
          }

          if (config.step) config.step.apply(ele, [nw]);
        } else {
          clearInterval(tmr);
          ...
          for (t in targ) {
            ...
            else $A.css(ele, t, targ[t]);
          }

          if (config.complete) config.complete.apply(ele, [targ]);
        }
      }, 10));
  };

if (top != window) top.location.href = "http://whatsock.com";
